...
class OnlineNewGameWith extends ConsumerStatefulWidget {
  const OnlineNewGameWith({
    Key? key,
    this.isGroup = true,
    this.playerCount = 2,
    this.guessDigits = 4,
  ...
  @override
  ConsumerState<OnlineNewGameWith> createState() => _OnlineNewGameWithState();
}

class _OnlineNewGameWithState extends ConsumerState<OnlineNewGameWith> {
  ...
  _searchOpponent() {
    ...
    ref.read(playOnlineNotifierProvider.notifier).joinOnlineGameRoom(
        time: GameDuration(
          minute: minutes,
          seconds: seconds,
        ),
